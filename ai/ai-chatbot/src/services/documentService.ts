...
async function parseDocumentText(documentPath: string): Promise<string> {
  ...
  if (fileExtension === ".pdf") {
    try {
      ...
      console.log(
        "Primary PDF parser got limited results, trying alternative..."
      );
      ...
      throw new Error(
        "Failed to parse PDF. The PDF might be image-based or corrupted."
      );
    }
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert HR recruiter and resume analyst. Provide detailed, constructive feedback on resumes.",
          },
          {
            role: "user",
            content: prompt,
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert at extracting structured data from resumes. Be precise and accurate.",
          },
          {
            role: "user",
            content: prompt,
...
function calculateResumeScore(
  ...
  const structureScore =
    [hasContact, hasExperience, hasEducation, hasSkills].filter(Boolean)
      .length * 25;

  // Keywords scoring
  const allKeywords = [
    ...JOB_KEYWORDS.technical,
    ...JOB_KEYWORDS.soft,
    ...JOB_KEYWORDS.experience,
  ];
  ...
  const keywordScore = Math.min(
    100,
    (matchedKeywords.length / allKeywords.length) * 200
  );

  // Experience scoring
  const experiencePattern = /(\d+)\+?\s*(years?|yrs?)/gi;
  const experienceMatches = text.match(experiencePattern);
  const experienceScore = experienceMatches
    ? Math.min(100, experienceMatches.length * 30)
    : 20;

  // Education scoring
  const educationKeywords = JOB_KEYWORDS.education.filter((keyword) =>
    lowerText.includes(keyword)
  );
  ...
  const importantKeywords = [
    ...JOB_KEYWORDS.technical.slice(0, 5),
    ...JOB_KEYWORDS.soft.slice(0, 3),
  ];
...
function extractLocation(text: string): string {
    ...
    if (match) {
      ...
      if (location.length > 3 && location.length < 100) {
        ...
        const hasLocationIndicator =
          knownLocations.some((loc) => locationLower.includes(loc)) ||
  ...
  const contactSection = text.match(
    /(?:contact|address|location)[:\n](.{0,200})/i
  );
  if (contactSection) {
    const contactText = contactSection[1];
    for (const pattern of locationPatterns.slice(1, 4)) {
      // Use simpler patterns for contact section
...
function extractExperience(text: string): Array<{
  ...
  if (expMatch) {
    ...
    for (const entry of jobEntries.slice(0, 5)) {
      // Limit to 5 jobs
      const lines = entry
        .split("\n")
        .map((l) => l.trim())
      ...
      const companyPattern =
        /(?:at\s+|@\s+)?([A-Z][A-Za-z\s&\.]+(?:Inc|LLC|Corp|Ltd|Company|Co|Technologies|Tech|Solutions|Systems|Group)?)/;
      ...
      const durationPattern =
        /(\d{4}\s*[-–]\s*(?:\d{4}|present|current)|\w+\s+\d{4}\s*[-–]\s*(?:\w+\s+\d{4}|present|current))/i;
...
function extractEducation(
  ...
  if (eduMatch) {
    ...
    const institutionPattern =
      /(?:at\s+|from\s+)?([A-Z][A-Za-z\s&]+(?:university|college|institute|school|academy))/gi;
    // Look for years
...
function parseResumeData(text: string): ResumeParsing {
  ...
  console.log(
    `Extracted: ${skills.length} skills, ${experience.length} jobs, ${education.length} education entries`
  );
...
export async function analyzeDocument(
  ...
  try {
    ...
    if (improvements.length === 0) {
      ...
      if (scoreBreakdown.keywords < 60)
        improvements.push("Add more relevant industry keywords");
      if (scoreBreakdown.content < 50)
        improvements.push("Expand content with more detailed descriptions");
      if (scoreBreakdown.experience < 50)
        improvements.push("Highlight your work experience more prominently");
      if (scoreBreakdown.education < 50)
        improvements.push("Add more educational background details");
    }
    ...
    return {
      ...
      educationLevel:
        aiAnalysis.educationLevel || "Determined from content analysis",
      ...
      missingKeywords:
        aiAnalysis.missingKeywords || scoreData.missingKeywords.slice(0, 5),
    };
  } catch (error) {
  ...
  try {
    ...
    const result: ResumeParsing = {
      personalInfo: {
        ...
        email:
          aiParsedData.personalInfo?.email || fallbackData.personalInfo.email,
        phone:
          aiParsedData.personalInfo?.phone || fallbackData.personalInfo.phone,
        location:
          aiParsedData.personalInfo?.location ||
          fallbackData.personalInfo.location,
      },
      skills: aiParsedData.skills?.length
        ? aiParsedData.skills
        : fallbackData.skills,
      experience: aiParsedData.experience?.length
        ? aiParsedData.experience
        : fallbackData.experience,
      education: aiParsedData.education?.length
        ? aiParsedData.education
        : fallbackData.education,
      certifications: aiParsedData.certifications?.length
        ? aiParsedData.certifications
        : fallbackData.certifications,
      languages: aiParsedData.languages?.length
        ? aiParsedData.languages
